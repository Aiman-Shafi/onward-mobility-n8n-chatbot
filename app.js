...
(function () {
  ...
  const defaultSettings = {
    webhook: {
      url: "https://n8n.cozmiq.com/webhook/2c705d76-4aff-4c94-be08-0e544b53a596/chat",
      route: "",
    },
    branding: {
      logo: "./sara.webp",
      name: "Sara - Your Mobility Assistant",
      welcomeText: "",
      responseTimeText: "",
      poweredBy: {
        text: "Powered by Onward Mobility",
        link: "https://onwardability.com",
      },
    },
    style: {
      primaryColor: "#63c8c0", // Green
      secondaryColor: "#359c97", // Darker green
      position: "right",
  ...
  const welcomeScreenHTML = `
        ...
        <div class="chat-welcome">
            <h2 class="chat-welcome-title"> 💬 Need help with our scooters, wheelchairs or specifications & questions? Looking for available coupons? 🎟️</h2>
        ...
        <div class="user-registration">
            <h2 class="registration-title">🌸 We’d love to help! Please enter your details to begin chatting.</h2> <br>
  ...
  function linkifyText(text) {
    ...
    // Convert URLs to HTML links or images
    text = text.replace(urlPattern, function (url) {
      // Check if the URL is a Shopify CDN URL (image URL)
      if (url.includes("cdn.shopify.com")) {
        return `<img src="${url}" alt="Product Image" style="max-width: 50%; margin-top: 8px; border-radius: 8px; border: 1px solid var(--chat-color-light);">`;
      } else {
        // Otherwise, treat it as a regular link
        return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="chat-link">View Product</a>`;
      }
    });

    // Bold text using Markdown syntax (e.g., **bold**)
    const boldPattern = /\*\*(.*?)\*\*/g;
    text = text.replace(boldPattern, function (match, p1) {
      return `<strong>${p1}</strong>`;
    });

    // Italic text using Markdown syntax (e.g., *italic*)
    const italicPattern = /\*(.*?)\*/g;
    text = text.replace(italicPattern, function (match, p1) {
      return `<em>${p1}</em>`;
    });

    // Strikethrough text using Markdown syntax (e.g., ~~strikethrough~~)
    const strikethroughPattern = /~~(.*?)~~/g;
    text = text.replace(strikethroughPattern, function (match, p1) {
      return `<del>${p1}</del>`;
    });

    // Link text using Markdown syntax (e.g., [Link](https://example.com))
    // const linkMarkdownPattern = /\[([^\[]+)\]\((https?:\/\/[^\)]+)\)/g;
    // text = text.replace(linkMarkdownPattern, function (match, p1, p2) {
    //   return `<a href="${p2}" target="_blank" rel="noopener noreferrer" class="chat-link">${p1}</a>`;
    // });

    // // Handle images with markdown-style: ![alt text](image_url)
    // const imagePattern = /!\[([^\]]+)\]\((https?:\/\/[^\)]+)\)/g;
    // text = text.replace(imagePattern, function (match, altText, imageUrl) {
    //   return `<img src="${imageUrl}" alt="${altText}" style="max-width: 100%; margin-top: 8px;">`;
    // });

    // Return the transformed text with links, bold, italics, and images
    return text;
  }
